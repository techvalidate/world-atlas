...
topo/world-%.json: shp/ne_%_land.shp shp/ne_%_admin_0_countries.shp
	mkdir -p $(dir $@)
	$(TOPOJSON) -o $@.tmp -q 1e4 --id-property=iso_a2,ISO3136 --properties=centroidLat=+LAT,centroidLng=+LONG -e country_centroids_all.tsv --no-stitch-poles -- land=shp/ne_$*_land.shp countries=shp/ne_$*_admin_0_countries.shp
